...
class ATMV2 {
	private int money;
	private final int LIMIT = 1000;	// LIMIT常數慣例為全大寫
...
class BankV2 extends Thread{
	private ATMV2 atm; // 擁有概念，故可存取之，與繼承不同
	...
	public void run() {
		...
		for (int i=0; i<10; i++) {
			synchronized (atm) {	// 鎖定ATM，調整為捕錢時，不允許其餘領錢動作
				atm.addMoney(200);
			}
//			atm.addMoney(200);
...
class PersonV2 extends Thread{
	...
	public void run() {
		for (int i=0; i<20; i++) {
			synchronized (name) {	// 鎖定ATM，調整為領錢時，不允許其餘補錢動作
